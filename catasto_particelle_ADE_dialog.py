...
from qgis.core import QgsProject, QgsVectorLayer, QgsCoordinateReferenceSystem, QgsFillSymbol
...
class ItalyInspireCadastreDownloaderDialog(QtWidgets.QDialog, FORM_CLASS):
    def __init__(self, parent=None):
        ...
        # pixmap1 = QPixmap(100, 50)
        pixmap1 = QPixmap(os.path.join(self.plugin_dir,'icons','rensolar.PNG'))  # Cargar imagen PNG
        # pixmap1_scaled = pixmap.scaledToWidth(50)
        self.label_svg.setMaximumWidth(225)
        self.label_svg.setMaximumHeight(67)
        self.label_svg.setPixmap(pixmap1)
        ...
        # self.comboBox_sheet.currentIndexChanged.connect(self.restart_progressbar)
    ...
    """def load_municipalities(self):
       # Cargar municipios basados en la provincia seleccionada.
        ...
        if region and province and province in self.json_data[region]:
            self.comboBox_sheet.addItem(self.tr("Scegli Foglio"), None)
            for municipality in self.json_data[region][province]:
                self.comboBox_sheet.addItem(municipality, municipality)
    ...
    def add_layers(self, folder, group_name):
        ...
        # Custom substring that identifies the file you want first
        priority_substring = "ple"  # Example: use part of the filename
        exclude_substring = "map"


        gml_files = [
            f for f in os.listdir(folder) 
            if f.endswith('.gml') and exclude_substring not in f
                     ]

        # Sort files by whether they contain the priority substring
        gml_files.sort(key=lambda f: (priority_substring not in f, f.lower()))
        ...
        for gmlfile in gml_files:
            ...
            gml_layer.setCrs(crs, True)
                ...
                # Apply symbology: no fill, black border
            symbol = QgsFillSymbol.createSimple({
                    'color': '0,0,0,0',       # Transparent fill (RGBA)
                    'outline_color': '0,0,255', # Blue border
                    'outline_width': '0.5'    # Optional: adjust border width
                })
            gml_layer.renderer().setSymbol(symbol)
            gml_layer.triggerRepaint()
    ...
    def download(self):
        ...
        # municipality = self.comboBox_sheet.currentData()
